...
export interface StoryRecommendationResult {
  story: NewsStory;
  score: number;
}

export interface StoryRecommendationResponse {
  strategy: string;
  results: StoryRecommendationResult[];
}
...
export async function getStories(params?: {
  ...
  if (params?.category) query.category = params.category;
  return fetchApi<StoryListResponse>("/stories", query);
}
...
export async function getStoryRecommendations(params?: {
  topic?: string;
  similar_to?: number;
  limit?: number;
}): Promise<StoryRecommendationResponse> {
  const query: Record<string, string> = {};
  if (params?.topic) query.topic = params.topic;
  if (params?.similar_to) query.similar_to = String(params.similar_to);
  if (params?.limit) query.limit = String(params.limit);
  return fetchApi<StoryRecommendationResponse>("/story-recommendations", query);
}
...
export async function getStoryFeed(userId: string, limit?: number): Promise<StoryRecommendationResponse> {
  const query: Record<string, string> = {};
  if (limit) query.limit = String(limit);
  return fetchApi<StoryRecommendationResponse>(`/story-feed/${userId}`, query);
}
